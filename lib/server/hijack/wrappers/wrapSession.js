...
export function wrapSession(sessionProto) {
  ...
  sessionProto.processMessage = function (msg) {
    ...
    if (msg.msg === 'method' || msg.msg === 'sub') {
      ...
      const startData = { userId: this.userId, params: JSON.stringify(msg.params) };
  ...
  sessionProto.protocol_handlers.method = function (msg, unblock) {
    ...
    if (kadiraInfo) {
      ...
      const newUnblock = Kadira.waitTimeBuilder.trackWaitTime(this, msg, unblock);
      response = Kadira.env.kadiraInfo.withValue(kadiraInfo, function () {
        return originalMethodHandler.call(self, msg, newUnblock);
      });
      newUnblock();
    } else {
  ...
  sessionProto.protocol_handlers.sub = function (msg, unblock) {
    ...
    if (kadiraInfo) {
      ...
      const newUnblock = Kadira.waitTimeBuilder.trackWaitTime(this, msg, unblock);
      response = Kadira.env.kadiraInfo.withValue(kadiraInfo, function () {
        return orginalSubHandler.call(self, msg, newUnblock);
      });
      newUnblock();
    } else {
  ...
  sessionProto.protocol_handlers.unsub = function (msg, unblock) {
    const newUnblock = Kadira.waitTimeBuilder.trackWaitTime(this, msg, unblock);
    const response = orginalUnSubHandler.call(this, msg, newUnblock);
    newUnblock();
    return response;
  };
  ...
  sessionProto.send = function (msg) {
    if (msg.msg === 'result') {
      ...
      if (kadiraInfo) {
        ...
        if (msg.error) {
          error = _.pick(msg.error, ['message', 'stack']);
