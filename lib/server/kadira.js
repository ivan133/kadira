...
const KadiraCore = Npm.require('kadira-core').Kadira;
import { WaitTimeBuilder } from './waitTimeBuilder';
import { MethodsModel } from './models/methodsModel';
import { SystemModel } from './models/systemModel';
import { PubsubModel } from './models/pubsubModel';
import { DocSzCache } from './docSizeCache';
import { Ntp } from '../common/ntp';
import { _ } from 'meteor/underscore';
import { ErrorModel } from './models/errorModel';
import { trackUncaughtExceptions, trackMeteorDebug } from './hijack/error';
...
Kadira.connect = function (appId, appSecret, options = {}) {
  ...
  const addFilterFn = Kadira.models.error.addFilter.bind(Kadira.models.error);
...
Kadira._buildPayload = function () {
  const payload = { host: Kadira.options.hostname };
  const buildDetailedInfo = Kadira._isDetailedInfo();
...
Kadira._sendAppStats = function () {
  ...
  _.each(Package, function (v, name) {
    appStats.packageVersions.push({ name, version: null });
  });
...
Kadira._getInfo = function (currentFiber, useEnvironmentVariable) {
  const fiber = currentFiber || Fibers.current;

  if (fiber) {
    if (useEnvironmentVariable) {
      return Kadira.env.kadiraInfo.get();
    }
    return fiber.__kadiraInfo;
  }
...
Kadira.trackError = function (type, message, options = {}) {
  if (Kadira.options.enableErrorTracking && type && message) {
    ...
    const error = { message: message, stack: options.stacks };
    const trace = {
      type: type,
